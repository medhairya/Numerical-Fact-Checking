...
def build_single_prompt(record: dict[str, Any]) -> str:
    ...
    docs_block = "\n\n".join(
        f"Document {idx}:\n{doc.strip()}" for idx, doc in enumerate(record["docs"][:5], start=1)
    )
...
**Step 2 — Check each decomposed question against the documents.**
...
Ask yourself: Am I defaulting to Conflicting out of uncertainty? If so, re-read the documents and force a lean toward True or False if any evidence points that way.
...
{{"label": "True|False|Conflicting", "reason": "1-2 sentence explanation citing the specific fact or document that decided the label"}}
"""
...
{docs_block}"""
    ...
    return f"""You are an expert numerical fact-checking assistant.

You will receive multiple fact-checking cases. For each case, assign exactly one label:
- True
- False
- Conflicting

Important instructions:
1. Focus strongly on numerical and date details: counts, years, percentages, durations, money, rankings, chronology, and comparisons.
2. If a key number or date is wrong, prefer False.
3. If evidence is mixed, too weak, too partial, or does not verify the key part of the claim, use Conflicting.
4. Use only the provided evidence documents.
5. Return one result per case.

Cases:
{cases_block}

Output ONLY valid JSON as an array of objects in this exact schema:
[{{"original_id":123,"label":"True|False|Conflicting","reason":"short explanation"}}]
"""
...
def predict_single_label(record: dict[str, Any]) -> str:
    ...
    return normalize_label(str(response_json.get("label", "")))
...
def predict_batch_labels(records: list[dict[str, Any]]) -> dict[int, str]:
    ...
    for item in response_json:
        ...
        if not isinstance(original_id, int):
            continue
        parsed[original_id] = normalize_label(str(item.get("label", "")))
